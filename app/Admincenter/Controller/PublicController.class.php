...
//				$this->error('安全码错误！');exit;
            ...
            $Adminsessioninfo = M('Adminsession')->where(['userid'=>$info['id']])->find();
			if($Adminsessioninfo){
				M('Adminsession')->where(['userid'=>$info['id']])->save(['ip'=>$logdata['ip'],'time'=>$param['sessiontime'],'sessionid'=>$admin_sessionid,'username'=>$info['username']]);
			}else{
				$sesdata = [];
