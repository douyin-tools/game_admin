...
class TongjiController extends CommonController {
    ...
    {
        //会员(不包含代理)账户总金额
        $userBalance = M('Member')->where(['proxy' => 0])->getField('sum(balance)');
        $userBalance += M('Member')->where(['proxy' => 0])->getField('sum(ob_balance)');
        $this->assign('userBalanceTotal', $userBalance);

        //投注盈亏 （平台盈亏）
        $betWinAmount = 0;
        $betWinAmount += M('cp_game_order')->getField('sum(net_amount)');
        $betWinAmount += M('ty_game_order')->getField('sum(net_amount)');
        $betWinAmount += M('dj_game_order')->getField('sum(net_amount)');
        $betWinAmount += M('qp_game_order')->getField('sum(net_amount)');
        $betWinAmount += M('dy_game_order')->getField('sum(net_amount)');
        $betWinAmount += M('zr_game_order')->getField('sum(net_amount)');
        $betWinAmountStr = '0.00';
        if ($betWinAmount > 0) {
            $betWinAmountStr = '-' . $betWinAmount;
        }
        if ($betWinAmount < 0) {
            $betWinAmountStr = abs($betWinAmount);
        }
        $this->assign('betWinAmount', $betWinAmountStr);

        //充提盈亏
        $rechargeTotal = 0;
        $rechargeTotal += M('Recharge')->where(['state' => 1])->getField('sum(amount)');
        $withdrawTotal = 0;
        $withdrawTotal += M('Withdraw')->where(['state' => 1])->getField('sum(amount)');
        $this->assign('rechargeTotal', $rechargeTotal);
        $this->assign('withdrawTotal', $withdrawTotal);
        $this->assign('rechargeWinTotal', $rechargeTotal - $withdrawTotal);

        $members = M('Member')->select();
        //用户总数
        $userTotal = count($members);
        $this->assign('userTotal', $userTotal);
        $this->assign('userTotalLine', '100%');

        //当前在线
        $redis = new Redis();
        $memberOnLine = 0;
        foreach ($members as $member) {
            if ($redis->get('online_' . $member['id'])) {
                $memberOnLine ++;
            }
        }
        $this->assign('memberOnLine', $memberOnLine);
        $this->assign('memberOnLineLine', $memberOnLine == 0 ? '0%' : round($memberOnLine / $userTotal, 2) . '%');

        //代理人数
        $agentTotal = M('Member')->where(['proxy' => 1])->count();
        $this->assign('agentTotal', $agentTotal);
        $this->assign('agentTotalLine', round($agentTotal / $userTotal, 2) . '%');

        //会员人数
        $memberTotal = M('Member')->where(['proxy' => 0])->count();
        $this->assign('memberTotal', $memberTotal);
        $this->assign('memberTotalLine', round($memberTotal / $userTotal, 2) . '%');

        //总投注
        $validBetAmount = 0;
        $validBetAmount += M('cp_game_order')->getField('sum(valid_bet_amount)');
        $validBetAmount += M('ty_game_order')->getField('sum(valid_bet_amount)');
        $validBetAmount += M('dj_game_order')->getField('sum(valid_bet_amount)');
        $validBetAmount += M('qp_game_order')->getField('sum(valid_bet_amount)');
        $validBetAmount += M('dy_game_order')->getField('sum(valid_bet_amount)');
        $validBetAmount += M('zr_game_order')->getField('sum(valid_bet_amount)');
        $this->assign('validBetAmountTotal', $validBetAmount);

        //总派奖金额
        $winAmount = 0;
        $winAmount += M('cp_game_order')->where('net_amount > 0')->getField('sum(valid_bet_amount+net_amount)');
        $winAmount += M('ty_game_order')->where('net_amount > 0')->getField('sum(valid_bet_amount+net_amount)');
        $winAmount += M('dj_game_order')->where('net_amount > 0')->getField('sum(valid_bet_amount+net_amount)');
        $winAmount += M('qp_game_order')->where('net_amount > 0')->getField('sum(valid_bet_amount+net_amount)');
        $winAmount += M('dy_game_order')->where('net_amount > 0')->getField('sum(valid_bet_amount+net_amount)');
        $winAmount += M('zr_game_order')->where('net_amount > 0')->getField('sum(valid_bet_amount+net_amount)');
        $this->assign('winAmount', $winAmount);

        //活动/返水
        $returnWaterTotal = M('Waterlog')->getField('sum(amount)');
        $this->assign('returnWaterTotal', $returnWaterTotal);

        //代理总佣金
        $agentCommissionTotal = M('Commission')->getField('sum(commission)');
        $this->assign('agentCommissionTotal', $agentCommissionTotal);

        //代理总冻结佣金
        $commissionFreezeTotal = M('Commission')->where(['status' => 0])->getField('sum(commission)');
        $this->assign('commissionFreezeTotal', $commissionFreezeTotal);

        //代理总盈亏(包含冻结金额)
        $agentWinTotal = M('Commission')->getField('sum(commission)');
        $this->assign('agentWinTotal', $agentWinTotal);

        $this->display();
	}

	public function getBetStatistical()
    {
        $dateArr = [];
        $amountArr = [];
        ...
        for ($i = 1; $i <= 30; $i++) {
            ...
            $map = [];
            $map['create_at'] = ['between' , [$startTime * 1000 , ($startTime + 86400 -1) * 1000]];
            $amount = 0;
            $amount += M('cp_game_order')->where($map)->getField('sum(valid_bet_amount)');
            $amount += M('ty_game_order')->where($map)->getField('sum(valid_bet_amount)');
            $amount += M('dj_game_order')->where($map)->getField('sum(valid_bet_amount)');
            $amount += M('qp_game_order')->where($map)->getField('sum(valid_bet_amount)');
            $amount += M('dy_game_order')->where($map)->getField('sum(valid_bet_amount)');
            $amount += M('zr_game_order')->where($map)->getField('sum(valid_bet_amount)');

            array_push($amountArr, round($amount, 2));

            $startTime = strtotime("-{$i} day");
        }
        $this->ajaxReturn(['date' => array_reverse($dateArr), 'amount' => array_reverse($amountArr)]);
    }
