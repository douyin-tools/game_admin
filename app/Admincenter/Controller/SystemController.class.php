...
class SystemController extends CommonController {
	...
	function del_huancun($dirName=""){
		
		$type=I('type');
		if(!in_array($type,['web_path','admin_path','kj_path']))$this->error('非法操作！');
		$dirName=empty($dirName)?GetVar($type).'/Runtime':$dirName;
         if ( $handle = opendir( "$dirName" ) ) {
			while ( false !== ( $item = readdir( $handle ) ) ) {
				if ( $item != "." && $item != ".." ) {
					if ( is_dir( "$dirName/$item" ) ) {
						self::del_huancun( "$dirName/$item" );
					} else {
	...
	function setting(){
		...
		foreach($setlist as $k=>$v){
			...
			if ($v['name'] == 'windows_client' || $v['name'] == 'mac_client' || $v['name'] == 'android_client' || $v['name'] == 'ios_client') {
                $newarray[$v['name'] . '_url'] = C('ADMIN_DOMAIN') . $v['value'];
            }
		}
	...
	function settingdo(){
        ...
        //app 二维码
        $android_qrcode = 'android_qrcode';
        $ios_qrcode = 'ios_qrcode';
		...
		foreach($param as $k=>$v){
		    if ($k == $android_qrcode || $k == $ios_qrcode || $k == $windows_client || $k == $mac_client || $k == $android_client || $k == $ios_client) {
		        continue;
            }
		...
		if (count($_FILES) > 0) {
		    foreach ($_FILES as $key => $file) {
		        $value = '';
                if ($key == 'android_qrcode' || $key == 'ios_qrcode') {
                    if ($file['error'] != 0) {
                        continue;
                    }
                    if($fp = fopen($file['tmp_name'],"rb", 0)) {
                        ...
                        $value = 'data:image/jpg/png/gif;base64,' . $value;
                    }
                }

                if ($key == 'android_client' || $key == 'ios_client' || $key == 'windows_client' || $key == 'mac_client') {
                    if ($file['error'] != 0) {
                        continue;
                    }

                    $value = '/static/client' . '/' . $file["name"];

                }

                if(!M('setting')->where(['name'=>$key])->find()){
                    $ints[] = M('setting')->data(['name' => $key, 'value' => $value])->add();
                }else{
                    $ints[] = M('setting')->where(['name' => $key])->setField(['value'=>$value]);
                }
            }
        }
