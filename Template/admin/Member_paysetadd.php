...
<script>
	KindEditor.ready(function(K) {
        ...
        var btn_trc20_ewmurlReadyTime = null;
        btn_trc20_ewmurlReadyTime = setInterval(function () {
                if($('#btn_trc20_ewmurl').length <= 0){
                    return;
                }else{
                    K("#btn_trc20_ewmurl").click(function() {
                        editor.loadPlugin('image', function() {
                            editor.plugin.imageDialog({
                                imageUrl : K("#trc20_ewmurl").val(),
                                clickFn : function(url, title, width, height, border, align) {
                                    K("#trc20_ewmurl").val(url);
                                    editor.hideDialog();
                                }
                            });
                        });
                    });
                    clearInterval(btn_trc20_ewmurlReadyTime);
                }
            }
            , 1000);

    var btn_erc20_ewmurlReadyTime = null;
    btn_erc20_ewmurlReadyTime = setInterval(function () {
            if($('#btn_ewmurl').length <= 0){
                return;
            }else{
                K("#btn_erc20_ewmurl").click(function() {
                    editor.loadPlugin('image', function() {
                        editor.plugin.imageDialog({
                            imageUrl : K("#erc20_ewmurl").val(),
                            clickFn : function(url, title, width, height, border, align) {
                                K("#erc20_ewmurl").val(url);
                                editor.hideDialog();
                            }
                        });
                    });
                });
                clearInterval(btn_erc20_ewmurlReadyTime);
            }
        }
        , 1000);
	});
